  ...
  data: () => ({
   ...
   methods: {
    ...
    async graficar(){
      ...
      try {
          const path_filtro = "/api/grafica-tipo-pago-rango?fechaFin="+this.fechaFin+"&fechaInicio="+this.fechaInicio
    ...
    async mounted () {
      this.loaded = false
      try {
        const response = await fetch('/api/grafica-tipo-pago').then(response => response.json())
